Nonogram.Gui = function ( puzzle, $container )
{
	var self = this;
...
Nonogram.Gui.prototype.drawPuzzle = function ( $puzzleContainer )
{
	var self           = this,
		html           = '',
		index          = 0,
	...
	html += '<table class="nonogram width-' + this.puzzle.width + ' height-' + this.puzzle.height + '">';
	html += '	<tr>';
	...
	$( document ).ready( function ()
	{
		...
		self.$puzzleContainer.find( '.nonogram td.hint.top .fill' ).each( function ()
		{
...
Nonogram.Gui.prototype.makePuzzlePlayable = function ()
{
	var self = this,
	...
	$( document ).ready( function ()
	{
		$table    = self.$puzzleContainer.find( '.nonogram' );
		$cells    = self.$puzzleContainer.find( '.nonogram td.puzzle-cell' );
		$allCells = self.$puzzleContainer.find( '.nonogram td' );
...
Nonogram.Gui.prototype.handleUserCellSelection = function ( cellIndex )
{
	var self  = this,
		$cell = self.$puzzleContainer.find( '.nonogram td.puzzle-cell[data-index="' + cellIndex + '"]' ),
		cell, cellValue;
	...
	if (self.playerMouseDown) {
		...
		if (self.lastPlayerModifiedCellIndex != cell.index) {
			...
			self.drawPreviewUserSolution( self.$puzzleContainer.find( '.nonogram .preview' ) );
...
Nonogram.Gui.prototype.setUserSolutionForCell = function ( cell, value )
{
	cell.userSolution = value;
};


Nonogram.Gui.prototype.drawSolution = function ()
{
	var self  = this,
		index = 0,
		row, column, $cell;
...
Nonogram.Gui.prototype.drawAiSolution = function ( $container )
{
	var self  = this,
		index = 0,
		cell, cssClass;
...
Nonogram.Gui.prototype.drawAiSolutionLog = function ( solver, $container )
{
	var self = this,
		logItem, i;
...
Nonogram.Gui.prototype.drawUserControls = function ( $container )
{
	var self = this,
		html = '';
	...
	html += '	<button id="reset-button" class="button icon reset" data-action="" data-foo="">Reset</button>';
	html += '	<button id="solve-button" class="button icon solve" data-action="" data-foo="">Solve</button>';
	html += '</div>';
	...
	$container.find( '.button.reset' ).click( function ()
	{
		...
		if (confirm( 'Are you sure you want to reset the puzzle?' )) {
			...
			self.$puzzleContainer.find( '.nonogram td.puzzle-cell' ).removeClass( 'user-solved user-positive user-negative solution-positive solution-negative' );
			self.$puzzleContainer.find( '.nonogram .preview' ).html( '' );
		}
	} );
	...
	$container.find( '.button.solve' ).click( function ()
	{
		...
		self.drawSolution();
		self.drawPreviewTrueSolution( self.$puzzleContainer.find( '.nonogram .preview' ) );
		//}
	} );
...
Nonogram.Gui.prototype.drawCreatorLog = function ( creator, $container )
{
	var html = '',
		logItem, i;
...
Nonogram.Gui.prototype.drawPreviewSolution = function ( $container, solutionType )
{
	var self  = this,
		index = 0,
		html  = '',
...
Nonogram.Gui.prototype.showPuzzleSolvedAnimation = function ()
{
	var self  = this,
		index = 0,
		interval,
	...
	{
		var $cell = self.$puzzleContainer.find( '.nonogram td.puzzle-cell[data-index="' + index + '"]' );
	...
	self.$puzzleContainer.find( '.nonogram' ).addClass( 'solved' );
};


Nonogram.Gui.prototype.drawPreviewUserSolution = function ( $container )
{
	this.drawPreviewSolution( $container, 'userSolution' );
};


Nonogram.Gui.prototype.drawPreviewAiSolution = function ( $container )
{
	this.drawPreviewSolution( $container, 'aiSolution' );
};

Nonogram.Gui.prototype.drawPreviewTrueSolution = function ( $container )
{
	this.drawPreviewSolution( $container, 'solution' );
};


Nonogram.Gui.prototype.sizeGrids = function ()
{
	var self = this;

	$( 'table.nonogram' ).each( function ()
	{
	...
	$( 'table.nonogram-preview' ).each( function ()
	{
