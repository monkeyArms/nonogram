...
LogicPic.Creator = function( puzzle )
{
	this.puzzle	= puzzle;
	this.log	= [];
};
...
LogicPic.Creator.prototype.create = function()
{
	...
	while(validPuzzle === false) {
		...
		solver = new LogicPic.Solver( this.puzzle );
...
LogicPic.Creator.prototype.getRandomIntBetween = function( min, max )
{
	min = Math.ceil( min );
	max = Math.floor( max );
